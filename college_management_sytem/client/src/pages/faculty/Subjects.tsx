...
export const FacultySubjects: React.FC = () => {
  ...
  if (loading) return <Loader label="Loading assigned subjects..." />;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 mt-1">
          Subjects assigned to you by the Academic Coordinator
        </p>
      </div>
      ...
      {subjects.length === 0 ? (
        <EmptyState
          ...
          description="You currently have no teaching subjects assigned by the coordinator."
          icon={BookOpen}
        />
      ) : (
            ...
            <Card key={sub.id || sub._id} className="relative flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  <Badge variant={sub.type === "Theory" ? "primary" : "warning"}>
                    {sub.type}
                  </Badge>
                </div>
